/**
 * display.js — Pure display computation.
 *
 * Takes road-matching and state data and returns values ready for the DOM or
 * MapLibre — colours, GeoJSON feature collections, badge items, SVG strings,
 * status text. No browser APIs, no DOM access. This separation keeps every
 * display decision unit-testable without a browser.
 *
 * Owns: computeTurnCard, segmentLineColor, overlayGeoJSON, segmentsGeoJSON,
 *   overlayBadgeItems, badgeItems, statusText, barState, turnPathSVG,
 *   junctionBranchGeoJSON, debugTripletFeatures.
 * Does not own: DOM writes, MapLibre API calls, state mutation — index.html.
 * junctionBranchGeoJSON(junctionGroups, matchedSegPts) always emits the blue
 *   matched-segment feature from matchedSegPts; skips entries with isMatched===true
 *   from junctionGroups to avoid drawing the matched segment twice.
 * overlayGeoJSON / overlayBadgeItems: when overlayAll=false, output is restricted
 *   to mainRoadPts; when overlayAll=true, all BFS segments are included.
 * turnPathSVG: projects BFS pts to local metres, rotates so pts[0]→pts[1] points
 *   up (car travels toward the top of the icon), flips y-axis for SVG, scales to
 *   fit the path bounding box including any curl-back below pts[0].
 */
