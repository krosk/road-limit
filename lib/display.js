...
/**
 * GeoJSON FeatureCollection for junction branch overlay.
 * junctionGroups: [{pts, isMatched}] from firstTurnAhead when reason='junction'.
 * Matched group drawn in blue, alternate branches in magenta.
 */
export function junctionBranchGeoJSON(junctionGroups) {
  if (!junctionGroups) return { type: 'FeatureCollection', features: [] };
  return {
    type: 'FeatureCollection',
    features: junctionGroups.map(({ pts, isMatched }) => ({
      type: 'Feature',
      geometry: { type: 'LineString', coordinates: pts },
      properties: { color: isMatched ? '#48f' : '#f48', isMatched: isMatched ? 1 : 0 },
    })),
  };
}
