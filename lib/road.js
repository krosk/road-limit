...
export function segmentsAhead(features, lon, lat, heading, maxDist, maxRadius = 2000, lookBehind = 0, matchMaxDist = 50) {
  ...
  while (queue.length > 0) {
    ...
    results.push({ pts, speedLimit, turn, turns, midpoint });
...
export function firstTurnAhead(features, lon, lat, heading, lookahead, maxTurnRadius, aThreshold, matchMaxDist = 50) {
  ...
  // Group segments by net bearing (pts[0] → pts[last]): segments within 30° of
  // each other are the same road split across OSM features (tile boundaries,
  // overlapping geometries, U-shape roads where the initial bearing misleads).
  // Only suppress the turn card if there are genuinely distinct directions ahead.
  // Also discard segments whose net bearing is more than 90° from the car's
  // heading: BFS always branches in both directions at every junction node, so
  // the "backward" branch (road the driver came from) always appears and must
  // be ignored before counting direction groups.
  ...
  for (const seg of forward) {
    ...
    if (b === null) continue;
    if (heading !== null && heading !== undefined && angleDiff(heading, b) > 90) continue;
    const group = dirGroups.find(g => Math.abs(angleDiff(b, g.bearing)) < 30);
    if (group) {
      if (seg.pts.length > group.seg.pts.length) { group.seg = seg; group.bearing = b; }
    } else {
  ...
  return {
    distanceToStart: cumdist[arcStartIdx],
