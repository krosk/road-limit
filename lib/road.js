...
export function segmentsAhead(features, lon, lat, heading, maxDist, maxRadius = 2000, lookBehind = 0, matchMaxDist = 50) {
  ...
  while (queue.length > 0) {
    ...
    const snapPtData = isMatched ? {
      snapPt: match.forward
        ? (match.segIdx > 0 ? match.coords[match.segIdx - 1] : null)
        : (match.segIdx < match.coords.length - 1 ? match.coords[match.segIdx + 1] : null),
    } : {};
    results.push({ pts, speedLimit, roadClass, isRamp, isMatched, turn, turns, midpoint, ...carPos, ...snapPtData });
  }
...
export function firstTurnAhead(segments, heading, lookahead, maxTurnRadius, aThreshold) {
  ...
  if (dirGroups.length > 1) {
    ...
    if (matchedGroup) {
      ...
      if (continuations.length === 1) {
        ...
        matchedGroup.seg = { ...cont, pts: [...matchedGroup.seg.pts, ...cont.pts.slice(1)], snapPt: matchedGroup.seg.snapPt };
        dirGroups = dirGroups.filter(g => g !== continuations[0]);
      }
    }
  }
  if (dirGroups.length !== 1) return {
    pts: null, card: null, reason: 'junction', snapPt: null,
  ...
  const snapPt = selectedSeg.snapPt ?? null;
  if (pts.length < 3) return { pts, card: null, reason: 'straight', snapPt };
  ...
  if (firstTightIdx === -1 || cumdist[firstTightIdx] > lookahead) return { pts, card: null, reason: 'straight', snapPt };
