...
export function segmentsAhead(features, lon, lat, heading, maxDist, maxRadius = 2000, lookBehind = 0) {
  ...
  if (lookBehind > 0) {
    const behindStart = match.forward
      ? Math.max(0, match.segIdx - 1)
      : Math.min(match.coords.length - 1, match.segIdx + 1);
    const behindPts = collectAhead(match.coords, behindStart, !match.forward, lookBehind);
