...
export function detectAllTurns(pts, maxRadius = 2000) {
  ...
  while (i < pts.length - 1) {
    ...
    if (r <= maxRadius) {
      // Entered a curve — scan forward to find the tightest node in this stretch
      let minR = r;
      let minI = i;
      let j = i + 1;
      while (j < pts.length - 1 && circumradius(pts[j - 1], pts[j], pts[j + 1]) <= maxRadius) {
        const rj = circumradius(pts[j - 1], pts[j], pts[j + 1]);
        if (rj < minR) { minR = rj; minI = j; }
        j++;
      }
      const b1 = bearingTo(pts[minI - 1][1], pts[minI - 1][0], pts[minI][1], pts[minI][0]);
      const b2 = bearingTo(pts[minI][1], pts[minI][0], pts[minI + 1][1], pts[minI + 1][0]);
      turns.push({ distance: cumdist[minI], angle: angleDiff(b1, b2), radius: minR });
      i = j; // resume after this curve stretch
    } else {
      i++;
    }
  }
