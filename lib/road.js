/**
 * road.js — Road matching and BFS-based lookahead.
 *
 * Given a set of normalised OSM road features and a car position, this module
 * finds the nearest road (matchRoad), walks the road graph forward via BFS
 * (segmentsAhead), detects turns by circumradius (detectAllTurns), and selects
 * a single road ahead with a turn card or a suppression reason (firstTurnAhead).
 * It also normalises raw queryRenderedFeatures output before any other function
 * sees it (normaliseFeatures).
 *
 * Owns: matchRoad, segmentsAhead, firstTurnAhead, detectAllTurns, detectTurn,
 *   collectAhead, normaliseFeatures, corneringSpeed, parseSpeed, distToSegment,
 *   isSingleRoadAhead, and the JUNCTION_THRESH and MINOR_CLASSES constants.
 * Does not own: display formatting, SVG generation, GeoJSON overlay data — display.js.
 * All coordinate arrays are [lon, lat] (GeoJSON convention).
 * JUNCTION_THRESH is 5 m — do not raise; at 25 m parallel non-topological
 *   structures were spuriously connected (regressions: roads_43, roads_55).
 * matchRoad skips features with oneway===1 when car heading is contra-flow.
 * BFS queues oneway features in forward (coordinate-order) direction only.
 * firstTurnAhead uses refHeading (not raw heading) for the direction filter;
 *   when heading is null, refHeading falls back to the matched segment's bearing.
 * firstTurnAhead scans all nodes within lookahead for the global minimum radius —
 *   not just the first contiguous arc.
 * pts[0] of each BFS segment is an endpoint and never receives a turn badge.
 * collectAhead always includes the first node that exceeds the distance budget,
 *   so returned segments may extend slightly past maxDist. BFS junction detection
 *   is unaffected: the over-budget node's cumulative distance fails the
 *   junctionDistFromCar >= maxDist guard and does not spawn further hops.
 * normaliseFeatures must be applied to queryRenderedFeatures output before any
 *   other function in this module receives it.
 */
  ...
  for (const feature of features) {
    ...
    for (let i = 0; i < coords.length - 1; i++) {
      ...
      if (d < bestDist) {
        ...
        const headingForward = heading === null || heading === undefined
          ? true
          : angleDiff(heading, segBear) < 90;
        // Reject contra-flow matches on oneway roads: the car cannot legally be
        // travelling there, and forcing forward=true would start BFS from the
        // wrong end, producing spurious junction connections that pass the heading filter.
        if (feature.properties?.oneway === 1 && !headingForward) continue;
        bestDist = d;
        // Oneway features must always be walked in coordinate order.
        const forward = feature.properties?.oneway === 1 ? true : headingForward;
...
export function segmentsAhead(features, lon, lat, heading, maxDist, maxRadius = 2000, lookBehind = 0, matchMaxDist = 50) {
  ...
  const JUNCTION_THRESH = 5; // metres — node proximity to count as a junction
  ...
  while (queue.length > 0) {
    ...
    if (pts.length >= 1) {
      for (const other of features) {
        ...
        // Ramp endpoints (and tile-boundary stubs) often land on a main-road segment
        // midpoint rather than at a node. Check point-to-segment proximity so those
        // T-junctions are not missed by the node-only loop above.
        for (let ni = 0; ni < otherCoords.length - 1; ni++) {
          for (let pi = 0; pi < pts.length; pi++) {
            const d = distToSegment(pts[pi][0], pts[pi][1], otherCoords[ni], otherCoords[ni + 1]);
            if (d < bestDist) {
              bestDist = d;
              bestNodeIdx = ni;
              junctionDistFromCar = distFromCar + cumdist[pi];
...
export function firstTurnAhead(segments, heading, lookahead, maxTurnRadius, aThreshold) {
  ...
  // Minimum radius across the first arc AND any tighter arc further within lookahead.
  // The first arc determines distanceToStart and direction; later arcs can only tighten minSpeed.
  let minRadius = Infinity;
  for (let i = arcStartIdx; i < pts.length; i++) {
    if (cumdist[i] > lookahead) break;
