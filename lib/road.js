...
export function segmentsAhead(features, lon, lat, heading, maxDist, maxRadius = 2000, lookBehind = 0, matchMaxDist = 50) {
  ...
  while (queue.length > 0) {
    ...
    results.push({ pts, speedLimit, roadClass, isRamp, isMatched, turn, turns, midpoint });
  }
...
export function firstTurnAhead(features, lon, lat, heading, lookahead, maxTurnRadius, aThreshold, matchMaxDist = 50) {
  ...
  for (const seg of forward) {
    ...
    if (group) {
      ...
      if (classBetter || rampBetter || (classSame && rampSame && seg.pts.length > group.seg.pts.length)) {
  ...
  if (dirGroups.length > 1) {
    ...
    if (matchedGroup) {
      ...
      const continuations = dirGroups.filter(g =>
        g !== matchedGroup &&
        haversine(end[1], end[0], g.seg.pts[0][1], g.seg.pts[0][0]) < 25
      );
  ...
  if (dirGroups.length !== 1) return { pts: null, card: null, reason: 'junction' };
