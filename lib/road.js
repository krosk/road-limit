...
export function firstTurnAhead(features, lon, lat, heading, lookahead, maxTurnRadius, aThreshold, matchMaxDist = 50) {
  ...
  // Group segments by initial bearing: segments within 30° of each other are the
  // same road split across OSM features (tile boundaries, overlapping geometries).
  // Only suppress the turn card if there are genuinely distinct directions ahead.
  const segBearing = seg =>
    seg.pts.length >= 2
      ? bearingTo(seg.pts[0][1], seg.pts[0][0], seg.pts[1][1], seg.pts[1][0])
      : null;
  const dirGroups = [];
  for (const seg of forward) {
    const b = segBearing(seg);
    if (b === null) continue;
