 ...
 * Returns { card, reason } where:
 *   card  — { pts, distanceToStart, arcLength, minRadius, minSpeed, direction } or null
...
export function firstTurnAhead(features, lon, lat, heading, lookahead, maxTurnRadius, aThreshold, matchMaxDist = 50) {
  ...
  if (forward.length === 0) return { card: null, reason: 'no match' };
  ...
  if (dirGroups.length !== 1) return { card: null, reason: 'junction' };

  const pts = dirGroups[0].seg.pts;
  if (pts.length < 3) return { card: null, reason: 'straight' };
  ...
  if (firstTightIdx === -1 || cumdist[firstTightIdx] > lookahead) return { card: null, reason: 'straight' };
