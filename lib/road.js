...
export function segmentsAhead(features, lon, lat, heading, maxDist, maxRadius = 2000, lookBehind = 0, matchMaxDist = 50) {
  ...
  while (queue.length > 0) {
    ...
    // Discard segments going backward relative to heading.  BFS always branches
    // both ways at junction nodes; the branch toward where the driver came from
    // has a net bearing >90° from heading and must not appear in the overlay.
    if (heading !== null && heading !== undefined) {
      const netB = bearingTo(pts[0][1], pts[0][0], pts[pts.length - 1][1], pts[pts.length - 1][0]);
      if (angleDiff(heading, netB) > 90) continue;
    }
...
export function firstTurnAhead(features, lon, lat, heading, lookahead, maxTurnRadius, aThreshold, matchMaxDist = 50) {
  ...
  const MINOR_CLASSES = new Set(['track', 'path', 'footway', 'cycleway', 'steps', 'bridleway']);
