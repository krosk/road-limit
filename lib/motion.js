 ...
 *
 * Converts raw devicemotion accelerometer readings into car-frame lateral and
 * longitudinal G-force. The approach uses the device orientation angles (β/γ)
 * to rotate device-frame acceleration into world frame (east/north/up), then
 * applies GPS heading to rotate world frame into car frame. No manual
 * calibration step is required; the transform self-corrects as the phone shifts.
 *
 * Owns: gravityInDeviceFrame, removeGravity, phoneToWorldFrame, worldToCarFrame,
 *   applyEMA.
 * Does not own: compass heading logic (state.js), sensor event binding (index.html).
 * No dependencies on other lib modules.
 * phoneToWorldFrame removes gravity then rotates device frame → world frame.
 * worldToCarFrame rotates world frame → car frame using GPS heading.
 * applyEMA is a linear EMA; the circular EMA for compass bearings is smoothCompass
 *   in state.js — do not use applyEMA for angles that wrap at 360°.
 */
const G = 9.81;
