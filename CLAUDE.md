...
- **Top-left overlay**: speed (km/h), road name (hidden when empty).
- **Top-right overlay**: turn summary card — SVG polyline tracing the actual
  road geometry ahead (white, generated by `turnPathSVG`), min cornering speed
  (red when over limit), distance and
